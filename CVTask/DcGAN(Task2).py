...
tags = ['Airplane', 'Automobile', 'Bird', 'Cat', 'Deer', 'Dog', 'Frog', 'Horse', 'Ship', 'Truck']
 # To show samples increase in quality as training progresses  
...
# Code usd to save the images in PC
from PIL import Image  
import PIL
for classlabel in range(9,10):
    lbls = one_hot_encode([classlabel] * 9)
    for i in range(0,100):
        noise = generate_noise(9, 100)
        gen_imgs = generator.predict([noise, lbls])
        img = image.array_to_img(gen_imgs[1], scale=True)
        img.save('D:/OneDrive/Desktop/cifar10/truck/truck_%d.jpg'%i)
